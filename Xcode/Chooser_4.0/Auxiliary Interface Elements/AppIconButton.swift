...
//  Chooser_4.0
//
//  Created by Matt Marks on 7/8/19.
...
class AppIconButton: UIControl {
    ...
    /// The CAShapeLayer that holds the circle in the middle of the icon.
    private var circleLayer = CAShapeLayer()
    
    /// If the icon is chosen, this is true. Otherwise, false.
    private var isChosen: Bool = false
    
    private enum Constants {
        static let circleColor: CGColor           = UIColor.white.cgColor
    ...
    /// Creates and adds the CAShapeLayer representing the circle to the button.
    private func createCircleLayer() {
        ...
        circleLayer.path = UIBezierPath(arcCenter: CGPoint(x: bounds.midX, y: bounds.midY),
                                        radius: circleLayer.bounds.width * 0.3,
                                        startAngle: 0,
                                        endAngle: 2 * .pi,
                                        clockwise: true).cgPath
        circleLayer.lineWidth = circleLayer.bounds.width * 0.1
        circleLayer.strokeColor = UIColor.white.cgColor
        circleLayer.fillColor = UIColor.clear.cgColor
        layer.insertSublayer(circleLayer, above: gradientLayer)
    }
